    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="/receptionist/admissions">
...
<div class="container-fluid mt-4">
    ...
    <div class="welcome-header">
        <div class="row align-items-center">
            <div class="col-md-8">
                <h2 class="mb-1">
                    <i class="fas fa-procedures me-2"></i>Admissions Management
                </h2>
                <p class="text-muted mb-0">Manage patient admissions and discharges</p>
            </div>
<%--            --%>
        </div>
    </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">
                        <i class="fas fa-bed me-2"></i>Current Admissions
                        <span class="badge bg-light text-primary ms-2">${currentAdmissions.size()}</span>
                    </h5>
                </div>
                <div class="card-body">
                    <c:choose>
                        <c:when test="${not empty currentAdmissions}">
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <c:forEach var="admission" items="${currentAdmissions}">
                                        <tr>
                                            <td>
                                                <div class="fw-bold">${admission.patient.firstName} ${admission.patient.lastName}</div>
                                                <small class="text-muted">
                                                    <i class="fas fa-phone me-1"></i>${admission.patient.contactNumber}
                                                </small>
                                            </td>
                                            <td>
                                                <div class="fw-bold">${admission.ward.wardNumber}</div>
                                                <small class="text-muted">Bed ${admission.bed.bedNumber}</small>
                                            </td>
                                            <td>
                                                <!-- Fixed LocalDateTime formatting -->
                                                    ${admission.admissionDate.toLocalDate()} ${admission.admissionDate.toLocalTime()}
                                            </td>
                                            <td>${admission.reason}</td>
                                            <td>
                                                <span class="badge bg-success status-badge">${admission.status}</span>
                                            </td>
                                            <td>
                                                <form action="/receptionist/admissions/discharge" method="post" class="d-inline">
                                                    <input type="hidden" name="admissionId" value="${admission.id}">
                                                    <button type="submit" class="btn btn-sm btn-outline-danger"
                                                            onclick="return confirm('Are you sure you want to discharge ${admission.patient.firstName} ${admission.patient.lastName}?')">
                                                        <i class="fas fa-sign-out-alt me-1"></i>Discharge
                                                    </button>
                                                </form>
                                                <a href="/receptionist/patients/${admission.patient.id}" class="btn btn-sm btn-outline-primary ms-1">
                        ...
                        <c:otherwise>
                            <div class="text-center py-5 text-muted">
                                ...
                                <p class="mb-3">There are no patients currently admitted to the hospital.</p>
                                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#admitPatientModal">
        ...
        <div class="col-md-3 mb-4">
            <div class="card border-primary card-hover">
                <div class="card-body text-center">
                    ...
                    <h3 class="text-primary">${currentAdmissions.size()}</h3>
                    <p class="text-muted mb-0">Current Admissions</p>
                </div>
            </div>
        </div>
        <div class="col-md-3 mb-4">
        ...
        <div class="col-md-3 mb-4">
            <div class="card border-info card-hover">
                <div class="card-body text-center">
                    ...
                    <h3 class="text-info">${wards.size()}</h3>
        ...
        <div class="col-md-3 mb-4">
            <div class="card border-warning card-hover">
                <div class="card-body text-center">
                    ...
                    <h3 class="text-warning">${patients.size()}</h3>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="admitPatientModalLabel">
                    <i class="fas fa-user-plus me-2"></i>Admit Patient
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="/receptionist/admissions/admit" method="post">
                <div class="modal-body">
                    <div class="row">
                            ...
                            <select class="form-select" id="patientId" name="patientId" required>
                                ...
                                <c:forEach var="patient" items="${patients}">
                                    <option value="${patient.id}">
                                        ...
                                        <c:if test="${not empty patient.contactNumber}">- ${patient.contactNumber}</c:if>
                                    </option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="wardId" name="wardId" required onchange="updateAvailableBeds(this.value)">
                                <option value="">Choose a ward...</option>
                                <c:forEach var="ward" items="${wards}">
                                    <option value="${ward.id}" data-available-beds="${ward.availableBeds}">
                                            ${ward.wardNumber} - ${ward.wardType}
                                        <c:if test="${ward.availableBeds > 0}">(${ward.availableBeds} beds available)</c:if>
                                        <c:if test="${ward.availableBeds == 0}">(Full)</c:if>
                                    </option>
                                </c:forEach>
                            </select>
                            <div class="form-text" id="wardInfo"></div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 mb-3">
                            <label for="reason" class="form-label">Admission Reason *</label>
                    ...
                    <div class="alert alert-info">
                        ...
                        <strong>Note:</strong> The system will automatically assign the first available bed in the selected ward.
                    </div>
                </div>
...
<script>
    ...
    // Update ward information when selection changes
    function updateAvailableBeds(wardId) {
        ...
        if (availableBeds > 0) {
            wardInfo.innerHTML = `<i class="fas fa-check text-success me-1"></i>${availableBeds} bed(s) available in this ward`;
            wardInfo.className = 'form-text text-success';
        } else if (availableBeds == 0) {
            wardInfo.innerHTML = `<i class="fas fa-exclamation-triangle text-warning me-1"></i>No available beds in this ward`;
            wardInfo.className = 'form-text text-warning';
        } else {
            wardInfo.innerHTML = '';
            wardInfo.className = 'form-text';
        }
    }

    // Form validation
    document.addEventListener('DOMContentLoaded', function() {
        const admitForm = document.querySelector('form[action="/receptionist/admissions/admit"]');
        if (admitForm) {
            admitForm.addEventListener('submit', function(e) {
                const wardSelect = document.getElementById('wardId');
                const selectedOption = wardSelect.options[wardSelect.selectedIndex];
                const availableBeds = parseInt(selectedOption.getAttribute('data-available-beds'));

                if (availableBeds === 0) {
                    e.preventDefault();
                    alert('Cannot admit patient: Selected ward has no available beds. Please choose a different ward.');
                    return false;
                }
            });
        }
    ...
    // Initialize ward info on modal show
    const admitModal = document.getElementById('admitPatientModal');
    if (admitModal) {
        admitModal.addEventListener('show.bs.modal', function() {
            const wardSelect = document.getElementById('wardId');
            if (wardSelect.value) {
                updateAvailableBeds(wardSelect.value);
            }
        });
    }
